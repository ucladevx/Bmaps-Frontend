...
export class DayComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this._eventService.currentView$.subscribe(view => {
      this.isDayView = (view == ViewState.day);
    ...
    this._eventService.filteredDayEvents$.subscribe(dayEventCollection => {
      this.filteredEvents = dayEventCollection.features;
      this.fillEventsByDay();
      this.ngZone.run( () => { this.updateCalendar(this._eventService.getSelectedDate()); });
    });
    ...
    // set current view
    this._eventService.setCurrentView(ViewState.day);

  }

  // display the calendar
  updateCalendar(dateInMonth: Moment | Date | string): void {
    if(!this.isDayView || dateInMonth == undefined)
      return;
