...
export class MapBoxComponent implements OnInit {
  ...
  addPinToLocation(id: string, latitude: number, longitude: number, icon: string, size: number, visible = true) {
    let point: FeatureCollection = {
      ...
      "features": [new GeoJson(id, {latitude, longitude})]
    };
  ...
  threeDDisplay(): void {
    ...
    this.map.addLayer({
      'id': 'ucla-buildings',
      'source': 'composite',
      'source-layer': 'UCLA_Campus_Buildings',
      'filter': ['==', 'extrude', 'true'],
      'type': 'fill-extrusion',
      'minzoom': 15,
      'paint': {
  ...
  addArrowControls(): void {
    ...
    // change in map zoom when the zoom buttons are clicked
    const deltaZoom = .5;
  ...
  // add popup to a mapbox pin, sections for every event in that location
  ...
  // Add hover behavior to pin popup
  addPopupBehavior(): void {
    // HOVER
  ...
  // bold popup event title for given event, unbold all other event titles
  ...
  addHoverBehavior(eventPopup, event){
    let bold = (e: MouseEvent|TouchEvent): void => { this.boldPopup(event); };
    let unbold = (e: MouseEvent|TouchEvent): void => { this.boldPopup(null); };
  ...
  listEventsByLocation(location : string){
    ...
    for(let eventIndex in this.events.features){
      ...
      if(evLocation === location){ eventList.push(ev); }
    }
